...
import AssistantModal, { type Assistant } from "../components/AssistantModal";
...
const COLUMNS = [
  { key: "id", label: "ID" },
  { key: "name", label: "F.I.Sh", filter: true },
  { key: "courses", label: "Biriktirilgan kurs", filter: true },
  { key: "phone", label: "Telefon raqami", sortable: true },
  { key: "role", label: "Rol", sortable: true },
  { key: "createdAt", label: "Yaratilgan vaqt", sortable: true },
  { key: "status", label: "Holat", filter: true },
...
/* [{name:"Frontend"},{name:"Vali"}] -> "Frontend, Vali" */
function courseNames(courses?: { name: string }[]) {
  const list = (courses ?? []).map((c) => c.name).filter(Boolean);
...
type LoadResult =
  | { kind: "ok"; rows: Assistant[] }
  | { kind: "unauthorized" }
  | { kind: "error"; message: string };

async function fetchAssistants(token: string): Promise<LoadResult> {
  const res = await fetch(`${API_ASSISTANTS}/all`, {
    headers: { Authorization: `Bearer ${token}` },
  });

  const body = await readBody<{ data?: Assistant[] } & ApiError>(res);
...
function ViewModal({
  ...
  const info: [string, string][] = [
    ["Biriktirilgan kurs", courseNames(assistant.courses)],
    ["Telefon raqami", formatPhone(assistant.phone) || "-"],
    ["Email", assistant.email || "-"],
    ["Rol", ROLE_LABEL[assistant.role] ?? assistant.role],
    ["Ro'yxatdan o'tgan vaqti", formatDate(assistant.create_at)],
    ["Oxirgi o'zgarish", formatDate(assistant.update_at)],
    ["Holat", assistant.status === "ACTIVE" ? "Faol" : "Nofaol"],
  ];
    ...
    <div className="v-ov" onClick={onClose}>
      <div className="v-md" onClick={(e) => e.stopPropagation()}>
        <div className="v-head">
          <div className="v-title">Assistent haqida</div>
        ...
        <div className="v-user">
          <Avatar file={assistant.file} name={assistant.full_name} size={76} />
          <div>
            <div className="v-name">{assistant.full_name}</div>
            <div className="v-job">ID: {assistant.id}</div>
          </div>
        </div>
...
export default function AssistantsPage() {
  ...
  const load = useCallback(() => {
    ...
    return fetchAssistants(token)
      .then((r) => {
        ...
        setAssistants(r.rows);
        setError("");
      })
      .catch((e: unknown) => {
        console.error("Assistentlarni yuklashda xato", e);
  ...
  const handleDelete = async (a: Assistant) => {
    ...
    setDeletingId(a.id);
    try {
      const res = await fetch(`${API_ASSISTANTS}/${a.id}`, {
        method: "DELETE",
  ...
  const filtered = useMemo(() => {
    ...
    return assistants.filter((a) =>
      [a.full_name, a.phone, a.email ?? "", courseNames(a.courses), String(a.id)]
        .filter(Boolean)
  ...
  return (
    <DashboardShell>
      <style>{`
        ...
        table.grid { width:100%; border-collapse:collapse; min-width:1280px; background:#fff; }
        ...
        .v-md { background:#fff; border-radius:16px; width:100%; max-width:640px; padding:24px 30px 28px;
      ...
      <div className="page-head">
        <div>
          <div className="page-title">Assistentlar</div>
          <div className="crumb">
            <span>Foydalanuvchilar</span>
            <i className="dot" />
            <span>Assistentlar</span>
          </div>
        </div>
                  ...
                  <tr key={a.id}>
                    ...
                    <td>
                      <div className="cell-user">
                        {/* rasm bo'lmasa ism bosh harfi ko'rsatiladi */}
                        <Avatar file={a.file} name={a.full_name} size={30} />
                        <span>{a.full_name}</span>
                      </div>
                    </td>
                    <td>{courseNames(a.courses)}</td>
                    <td>{formatPhone(a.phone)}</td>
                    <td>{ROLE_LABEL[a.role] ?? a.role}</td>
                    <td>{formatDate(a.create_at)}</td>
                    <td>
                      <span className={`badge ${a.status === "ACTIVE" ? "faol" : "nofaol"}`}>
