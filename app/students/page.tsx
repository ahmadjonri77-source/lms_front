...
import StudentModal, { type Student } from "../components/StudentModal";
...
type LoadResult =
  | { kind: "ok"; rows: Student[] }
  | { kind: "unauthorized" }
  | { kind: "error"; message: string };

async function fetchStudents(token: string): Promise<LoadResult> {
  const res = await fetch(API_STUDENTS, {
    headers: { Authorization: `Bearer ${token}` },
  });

  const body = await readBody<{ data?: Student[] } & ApiError>(res);
...
function ViewModal({
  ...
  const info: [string, string][] = [
    ["Telefon raqami", formatPhone(student.phone) || "-"],
    ["Email", student.email || "-"],
    ["Rol", ROLE_LABEL[student.role] ?? student.role],
    ["Ro'yxatdan o'tgan vaqti", formatDate(student.create_at)],
    ["Oxirgi o'zgarish", formatDate(student.update_at)],
    ["Holat", student.status === "ACTIVE" ? "Faol" : "Nofaol"],
  ];
    ...
    <div className="v-ov" onClick={onClose}>
      <div className="v-md" onClick={(e) => e.stopPropagation()}>
        <div className="v-head">
          <div className="v-title">O&apos;quvchi haqida</div>
        ...
        <div className="v-user">
          <Avatar file={student.file} name={student.full_name} size={76} />
          <div>
            <div className="v-name">{student.full_name}</div>
            <div className="v-job">ID: {student.id}</div>
          </div>
        </div>
...
export default function StudentsPage() {
  ...
  const load = useCallback(() => {
    ...
    return fetchStudents(token)
      ...
      .catch((e: unknown) => {
        console.error("O'quvchilarni yuklashda xato", e);
  ...
  const handleDelete = async (s: Student) => {
    ...
    setDeletingId(s.id);
    try {
      const res = await fetch(`${API_STUDENTS}/${s.id}`, {
        method: "DELETE",
  ...
  const filtered = useMemo(() => {
    ...
    return students.filter((s) =>
      [s.full_name, s.phone, s.email ?? "", String(s.id)]
        .filter(Boolean)
  ...
  return (
    <DashboardShell>
      ...
      <div className="page-head">
        <div>
          <div className="page-title">O&apos;quvchilar</div>
          <div className="crumb">
                  ...
                  <tr key={s.id}>
                    ...
                    <td>
                      <div className="cell-user">
                        ...
                        <Avatar file={s.file} name={s.full_name} size={30} />
                    ...
                    <td>
                      <span className={`badge ${s.status === "ACTIVE" ? "faol" : "nofaol"}`}>
