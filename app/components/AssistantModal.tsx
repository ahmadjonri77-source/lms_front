...
/* GET /api/v1/assistend/all qaytaradigan shakl */
export type Assistant = {
  ...
  create_at: string;
  update_at: string;
  /* backend biriktirilgan kurslarni shu yerda qaytaradi */
  courses?: { name: string }[];
};

type Props = {
  /* assistant berilsa — tahrirlash, bo'lmasa — qo'shish */
...
export default function AssistantModal({
  ...
  const currentImage = assistant?.file ? `/uploads/images/${assistant.file}` : "";

  const [fullName, setFullName] = useState(assistant?.full_name ?? "");
  const [phone, setPhone] = useState(assistant?.phone ?? "+998");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // assistend PATCH parolni hash qilib saqlaydi, shuning uchun tahrirda ham yuboriladi
      if (password) fd.append("password", password);

      const res = await fetch(
        isEdit ? `${API_ASSISTANTS}/${assistant!.id}` : API_ASSISTANTS,
        {
      ...
      console.error("Assistentni saqlashda xato", err);
