...
/* tahrirlash uchun kerak bo'ladigan minimal shakl */
...
type Props = {
  /* admin berilsa — tahrirlash, bo'lmasa — qo'shish.
  ...
  admin?: AdminRow | null;
  onClose: () => void;
  /* muvaffaqiyatli saqlangach ro'yxatni yangilash uchun */
  onSaved: () => void;
  /* token eskirgan bo'lsa /login ga qaytarish uchun */
  onUnauthorized: () => void;
};
...
export default function AddAdminModal({
  ...
  const isEdit = !!admin;
  const currentImage = admin?.file ? `/uploads/images/${admin.file}` : "";

  const [fullName, setFullName] = useState(admin?.full_name ?? "");
  const [phone, setPhone] = useState(admin?.phone ?? "+998");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit) {
        // Parol yuborilmaydi: backend PATCH'da uni hash qilmay saqlaydi
        res = await fetch(`${API_ADMINS}/${admin!.id}`, {
          method: "PATCH",
      ...
      console.error("Adminni saqlashda xato", err);
    ...
    <div className="ov" onClick={close}>
      ...
      <div className="md" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          {/* Rasm — qo'shishda ham, tahrirlashda ham */}
          <div className="fld">
            <div className="fld-label">
          ...
          {/* Parol — faqat qo'shishda (PATCH parolni hash qilmaydi) */}
          {!isEdit && (
            <div className="fld">
              <div className="fld-label">Parol</div>
